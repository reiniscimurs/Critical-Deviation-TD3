...
class Actor(nn.Module):
    ...
    def reward(self, batch_rewards, dev_action, device):
        rewards = []
        for i, b in enumerate(batch_rewards):
            override_action = dev_action[i]
            deviation = (override_action[0] ** 2) + (override_action[1] ** 2)
            rewards.append([b["collision"] + b["goal"] + b["step"] - deviation.item()])
...
class dualCNNTD3(object):
    ...
    def train(
        ...
        for it in range(iterations):
            ...
            av_base_Q += torch.mean(target_Q)
            max_base_Q = max(max_base_Q, torch.max(target_Q))
            with torch.no_grad():
                dev_action_updated = self.dev_actor_target(lidar, velocity, base_action)
                target_Q = self.actor.reward(
                    batch_rewards, dev_action_updated, self.device
            ...
            av_dev_Q += torch.mean(dev_target_Q)
            max_dev_Q = max(max_dev_Q, torch.max(dev_target_Q))
            with torch.no_grad():
            ...
            av_base_loss += loss
            av_dev_loss += dev_critic_loss
        self.iter_count += 1
        return {
            "base_loss": av_base_loss / iterations,
            "base_avg_Q": av_base_Q / iterations,
            "base_max_Q": max_base_Q,
            "dev_loss": av_dev_loss / iterations,
            "dev_avg_Q": av_dev_Q / iterations,
            "dev_max_Q": max_dev_Q,
        }
